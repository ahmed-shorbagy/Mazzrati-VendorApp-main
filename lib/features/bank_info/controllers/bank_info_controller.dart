...
import 'package:mazzraati_vendor_app/features/order/domain/models/business_analytics_filter_data.dart';
import 'package:mazzraati_vendor_app/features/profile/domain/models/profile_body.dart';
...
class BankInfoController extends ChangeNotifier {
  ...
  Future<void> getBankInfo(BuildContext context) async {
    ...
    // log("===bank_info_account_no==>${_bankInfo?.accountNo}");
    // log("===bank_info_bank_name==>${_bankInfo?.bankName}");
    // log("===bank_info_branch==>${_bankInfo?.branch}");
    // log("===bank_info_holder_name==>${_bankInfo?.holderName}");

    // log(_bankInfo!.toJson().toString());
    notifyListeners();
  }

  Future<bool> isBankInfoCompleted() async {
    _bankInfo = await bankInfoServiceInterface.getBankList();
    if (_bankInfo?.accountNo == null ||
        _bankInfo?.bankName == null ||
        _bankInfo?.branch == null ||
        _bankInfo?.holderName == null) {
      log("bank info incomplete");
      return false;
    } else {
      log("bank info complete");
      return true;
    }
  }
