...
class TransactionChartState extends State<TransactionChart> {
  final Color earningColor = const Color(0xFF4E9BF0);
  final Color commissionColor = const Color(0xFFF4BE37).withOpacity(.5);
  ...
  List<ChartData> _expenseChartList = [];
  List<ChartData> _incomeChartList = [];
  ...
  Widget build(BuildContext context) {
    return Consumer<BankInfoController>(
      builder: (context, bankInfoProvider, _) {
        ...
        if (earnings.length < 9) {
          _expenseChartList = commissions.asMap().entries.map((e) {
            return ChartData(weeks[e.key.toInt()], e.value);
          }).toList();
                ...
                Expanded(
                  child: LineChart(
                    LineChartData(
                      lineBarsData: [
                        LineChartBarData(
                          spots: _incomeChartList.asMap().entries.map((e) {
                        ...
                        LineChartBarData(
                          spots: _expenseChartList.asMap().entries.map((e) {
                            return FlSpot(e.key.toDouble(), e.value.y);
                          }).toList(),
                          isCurved: true,
                          color: earningColor,
                      ...
                      titlesData: FlTitlesData(
                        bottomTitles: AxisTitles(
                          sideTitles: SideTitles(
                            ...
                            // getTitles: (value) {
                            //   if (value < 0 ||
                            //       value >= _expenseChartList.length) {
                            //     return '';
                            //   }
                            //   return _expenseChartList[value.toInt()].x;
                            // },
                          ),
                        ),
                        leftTitles: AxisTitles(
                          sideTitles: SideTitles(
                            ...
                            // getTitles: (value) {
                            //   // Customize left axis titles if needed
                            //   return '${value.toInt()}';
                            // },
                          ),
                        ),
