...
class ProductController extends ChangeNotifier {
  ...
  List<Product> _suggestedProductList = [];
  List<Product> get suggestedProductList => _suggestedProductList;
  ...
  // Future<void> getAllSuggestedProduct() async {
  //   ApiResponse apiResponse =
  //       await productServiceInterface.getSuggestedProduct();

  //   if (apiResponse.response != null &&
  //       apiResponse.response!.statusCode == 200) {
  //     List<dynamic> data = apiResponse.response!.data;
  //     log(apiResponse.response!.data[0].toString());
  //     Logger().f(data);
  //     // _suggestedProductList =
  //     //     data.map((product) => Product.fromJson(product)).toList();
  //     _suggestedProductList = data.map((productjson) {
  //       Product product = Product.fromJson(productjson);
  //       product.categoryIds = (productjson['category_ids'] as List?)
  //           ?.map((e) => CategoryIds.fromJson(e))
  //           .toList();
  //       return product;
  //     }).toList();
  //   } else {
    ...
    if (apiResponse.response != null &&
      ...
      if (data is List) {
        _suggestedProductList = data.map((productJson) {
          ...
          // Safely parse category_ids from string to List<CategoryIds>
          var categoryIdsString = productJson['category_ids'] as String?;
          if (categoryIdsString != null) {
            List<dynamic> parsedList = jsonDecode(categoryIdsString);
            product.categoryIds =
                parsedList.map((e) => CategoryIds.fromJson(e)).toList();
          } else {
            product.categoryIds = [];
          }
        ...
        Logger().e("Unexpected data format: ${data.runtimeType}");
      }
    } else {
      ApiChecker.checkApi(apiResponse);
    }
