...
class AddProductRepository implements AddProductRepositoryInterface {
  ...
  Future<ApiResponse> addProduct(
    ...
    if (isDigitalVariationActive ?? false) {
      fields.addAll(<String, dynamic>{
        ...
        'digital_product_variant_key':
            jsonEncode(digitalVariationModel?.digitalVariantKeyMap),
        'digital_product_sku':
            jsonEncode(digitalVariationModel?.digitalVariantSku),
        'digital_product_price':
            jsonEncode(digitalVariationModel?.digitalVariantPrice),
      });

      if (digitalVariationModel?.variationType != null) {
        for (int i = 0; i < digitalVariationModel!.variationType!.length; i++) {
          fields.addAll(<String, dynamic>{
            'extensions_options_${digitalVariationModel.variationType![i]}':
    ...
    if (isDigitalVariationActive ?? false) {
      try {
        List<MultipartWithKey> multiPartFiles =
            await processItems(digitalVariationModel);
  ...
  Future<List<MultipartWithKey>> processItems(
      DigitalVariationModel? digitalVariationModel) async {
    List<MultipartWithKey> multipartBody = [];
    await Future.forEach(digitalVariationModel!.digitalVariantFiles!.keys,
        (key) async {
      if (digitalVariationModel.digitalVariantFiles![key] != null) {
        MultipartFile multiPartFile = MultipartFile.fromBytes(
          ...
          filename:
              basename(digitalVariationModel.digitalVariantFiles![key].name),
        );
        multipartBody.add(MultipartWithKey(
            key: 'digital_files_$key', multipartFile: multiPartFile));
      }
    });
  ...
  @override
  Future<ApiResponse> deleteDigitalVariationFile(
      int? productId, String variantKey) async {
    try {
